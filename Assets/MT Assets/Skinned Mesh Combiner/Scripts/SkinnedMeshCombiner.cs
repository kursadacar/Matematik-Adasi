...
namespace MTAssets.SkinnedMeshCombiner
{
    ...
    public class SkinnedMeshCombiner : MonoBehaviour
    {
        ...
        {
            public bool mergeOnlyEqualRootBones = true;
        }
        [Serializable]
        ...
        [HideInInspector]
        public bool ignoreBlendshapes = true;
        [HideInInspector]
        ...
        [HideInInspector]
        public bool clearUndoHistory = false;

        //Variables of interface in Editor
        [HideInInspector]
        public int currentTab = 0;
        [HideInInspector]
        public int currentDebuggingTab = 0;
        [HideInInspector]
        public Vector2 logsOfMergeScrollpos = Vector2.zero;
        [HideInInspector]
        public int lastQuantityOfLogs = 0;
        [HideInInspector]
        public bool dontShowWarnsAboutTexturesResolution = false;
        ...
        public class CustomInspector : UnityEditor.Editor
        {
            ...
            public override void OnInspectorGUI()
            {
                ...
                script.gizmosOfThisComponentIsDisabled = DisableGizmosInSceneView("SkinnedMeshCombiner", script.gizmosOfThisComponentIsDisabled);
            ...
            void Tab_Merge(SkinnedMeshCombiner script)
            {
                ...
                if (script.resultMergeGameObject == null)
                {
                    ...
                    script.combineOnStart = (bool)EditorGUILayout.Toggle(new GUIContent("Combine On Start",
                        "If you activate this option, the Skinned Mesh Combiner will automatically combine the meshes of this model, right at the beginning of the game."),
                        script.combineOnStart);
                    if (script.combineOnStart == true)
                    {
                    ...
                    script.ignoreBlendshapes = (bool)EditorGUILayout.Toggle(new GUIContent("Ignore Blendshapes",
                        "Activate this option so that the Skinned Mesh Combiner ignores the meshes that contain Blendshapes. Sometimes, combining meshs with Blendshapes may not give the desired results. If you still want to combine meshs with Blendshapes, disable this function!"),
                        script.ignoreBlendshapes);
                    ...
                    {
                        script.currentDebuggingTab = GUILayout.Toolbar(script.currentDebuggingTab, new string[] { "Skinned Meshes (" + debuggingGameObjects.Count + ")", "Materials (" + debuggingMaterials.Count + ")", "Textures (" + debuggingTextures.Count + ")" });
                        EditorGUILayout.BeginVertical("box");
                        debuggingScrollpos = EditorGUILayout.BeginScrollView(debuggingScrollpos, GUIStyle.none, GUI.skin.verticalScrollbar, GUILayout.Width(GetInspectorWindowSize().x), GUILayout.Height(100));
                    ...
                    if (Application.isPlaying == false)
                    {
                        ...
                        if (script.savePrefabOfMerge == true)
                        {
                            ...
                            if (script.nameOfPrefabOfMerge == "")
                            {
                                ...
                                script.nameOfPrefabOfMerge = "prefab_of_merge_" + now.Ticks;
                            }
                            EditorGUI.indentLevel -= 1;
                        }
                    }
                ...
                if (script.resultMergeGameObject == null)
                {
                    ...
                    //----- Run simple verifications to guarantee the quality of merge of textures, in atlas -----//
                    //If found textures with different resolutions
                    if (Tab_Merge_DebuggingTextures_AndVerifyTexturesSizes(true) == true && script.dontShowWarnsAboutTexturesResolution == false && script.mergeMethod == MergeMethod.AllInOne)
                    {
                        ...
                        if (GUILayout.Button("Ignore And Don't Warn To This Mesh Anymore"))
                            if (EditorUtility.DisplayDialog("Are you sure?", "Warnings for resolving texture problems will no longer be displayed for this Skinned Mesh Combiner component. The warning may appear on other components or new Skinned Mesh Combiner components created afterwards, without problems. These warnings are useful to help you avoid merging problems. Are you sure you want to hide any text resolution warnings for this Skinned Mesh Combiner component?", "Yes", "No") == true)
                                script.dontShowWarnsAboutTexturesResolution = true;
                    }
            ...
            void MergeParms_JustMaterialColorsParams(SkinnedMeshCombiner script)
            {
                ...
                if (script.justMaterialColorsParams.useDefaultColorProperty == false)
                {
                    ...
                    script.justMaterialColorsParams.colorPropertyToFind = script.DrawDropDownOfProperties("Find Colors In",
                        ...
                        (CurrentRP.haveAnotherSrpPackages == false) ? "_Color" : "_BaseColor", (CurrentRP.haveAnotherSrpPackages == false) ? "(Color Property)" : "(Base Color Property)", script.justMaterialColorsParams.colorPropertyToFind, propertiesOfAllMaterialsFounded);
            ...
            void MergeParms_AllInOneParams(SkinnedMeshCombiner script)
            {
                ...
                if (script.allInOneParams.useDefaultMainTextureProperty == false)
                {
                    ...
                    script.allInOneParams.mainTexturePropertyToFind = script.DrawDropDownOfProperties("Find Main Text. In",
                        "The name of the shader property, which is responsible for storing the Main Texture, in the material of its meshes. The Skinned Mesh Combiner will use the property here reported to fetch the metallic texture map on each material in each mesh. Usually the name used by most shaders is \"_MainTex\" or \"_BaseMap\" (HDRP and URP), but if any of your shaders have a different name, you can enter it here.\n\nIf the Skinned Mesh Combiner can not find the Main Texture in the mesh material, it will be without Main Texture after merging.",
                        (CurrentRP.haveAnotherSrpPackages == false) ? "_MainTex" : "_BaseMap", (CurrentRP.haveAnotherSrpPackages == false) ? "(Main Texture Property)" : "(Albedo Property)", script.allInOneParams.mainTexturePropertyToFind, propertiesOfAllMaterialsFounded);
                    ...
                    script.allInOneParams.mainTexturePropertyToInsert = script.DrawDropDownOfProperties("Apply Main Text. In",
                        ...
                        (CurrentRP.haveAnotherSrpPackages == false) ? "_MainTex" : "_BaseMap", (CurrentRP.haveAnotherSrpPackages == false) ? "(Main Texture Property)" : "(Albedo Property)", script.allInOneParams.mainTexturePropertyToInsert, propertiesOfMaterialToUse);
                    EditorGUI.indentLevel -= 1;
                }
    ...
    EditorGUILayout.HelpBox("This merge method works only with meshes generated from Sprites 2D, using the Anima2D tool from Unity Technologies. The Skinned Mesh Combiner has not detected the namespace of this tool anywhere in your project. Please install Anima2D before using this merge method. If you're not going to use Anima2D, consider using other standard merge methods (facing 3D meshes).", MessageType.Error);
            ...
            void Tab_Stats(SkinnedMeshCombiner script)
            {
                ...
                if (Application.isPlaying == false)
                {
                    if (script.resultMergeGameObject == null)
                    {
                        if (CurrentRP.haveAnotherSrpPackages == true)
                            EditorGUILayout.HelpBox("The Skinned Mesh Combiner has identified that you are using another Scriptable Render Pipeline package. Apparently you're using " + CurrentRP.packageDetected + ".", MessageType.Info);
                        if (CurrentRP.haveAnotherSrpPackages == false)
            ...
            void Tab_LogsOfMerge(SkinnedMeshCombiner script)
            {
                ...
                EditorGUILayout.BeginVertical("box", GUILayout.Width(GetInspectorWindowSize().x), GUILayout.Height(300));
        ...
        private bool isMissingDataOfMergeOrExistsProblemWithMerge()
        {
            ...
            if (renderer != null)
            {
                foreach (Material mat in renderer.sharedMaterials)
                {
                    ...
                    if (mat.GetTexture((CurrentRP.haveAnotherSrpPackages == true) ? "_BaseMap" : "_MainTex") == null && mergeMethod != MergeMethod.OnlyAnima2dMeshes && mergeMethod != MergeMethod.OneMeshPerMaterial)
                    {
                        return true;
                    }
                }
        ...
        void Start()
        {
            //Validate all variables on start
            ValidateAllVariables();

            //If is desired, combine meshes at start
            if (combineOnStart == true && Application.isPlaying == true)
                CombineMeshes();
            if (combineOnStart == true && convertCombinedMeshToStaticOnStart == true && Application.isPlaying == true)
        ...
        public void CombineMeshes()
        {
            ...
            GetCombinedMeshSkinnedMeshRenderer().gameObject.layer = LayerMask.NameToLayer("Player");
        }
        ...
        public void UndoCombineMeshes(bool runUnityGc, bool runMonoIl2CppGc)
        {
            ...
            if (isCombiendMeshesConvertedToStatic() == true)
            {
        ...
        public bool isCombiendMeshesConvertedToStatic()
        {
        ...
        private void DoCombineMeshs_OneMeshPerMaterial()
        {
            ...
            try
            {
                ...
                //Change the animator to always animate, and store the original value
                Animator thisAnimator = GetComponent<Animator>();
                AnimatorCullingMode originalCullingMode = thisAnimator.cullingMode;
                ...
                smrender.rootBone = skinnedMeshesToMerge[0].rootBone;
                ...
                //Show alert e log
                ShowUnityAlertDialog("Done", "The merging of the meshes was completed successfully!", "Ok");
            ...
            try
            {
                ...
                foreach (SkinnedMeshRenderer meshRender in skinnedMeshesToMerge)
                {
                    ...
                    for (int i = 0; i < meshRender.sharedMesh.subMeshCount; i++)
                    {
                        ...
                        int uvVerticesCount = combineInstance.mesh.GetSubmesh(i).vertexCount;
                        ...
                        uvDataOfThisSubmesh.originalUvVertices = new Vector2[uvVerticesCount];
                        ...
                        materialCount += 1;
                        originalUvLenght += combineInstance.mesh.GetSubmesh(i).uv.Length;

                        //Add the total vertices verified
                        totalVerticesVerifiedAtHere += uvVerticesCount;
                    }
                }
                ...
                List<Vector2> newUvMapForCombinedMesh = new List<Vector2>();
                foreach (UvDataAndColorOfThisSubmesh thisUvData in uvDatasToMerge)
                {
                    //Get and copy all original respective UV vertices of combined meshes, but only of this submesh
                    for (int i = 0; i < thisUvData.originalUvVertices.Length; i++)
                        thisUvData.originalUvVertices[i] = smrender.sharedMesh.uv[thisUvData.startOfUvVerticesIndex + i];
                    ...
                    for (int i = 0; i < thisUvData.originalUvVertices.Length; i++)
                    {
                        ...
                        newUvMapForCombinedMesh.Add(thisVertex);
                    }
                }
                ...
                //If the new UV map not matches with lenght of original
                if (smrender.sharedMesh.uv.Length != newUvMapForCombinedMesh.Count())
                {
                    LaunchLog("Could not proceed with the merge. For some reason, the new UV mesh generated, does not match the UV mesh of this character... So the merging was interrupted, to avoid mapping errors. Please contact MT Assets support at mtassets@windsoft.xyz.\n\nThe new UV map have " + newUvMapForCombinedMesh.Count + " vertices and the original UV map have " + smrender.sharedMesh.uv.Length + " vertices.", LogTypeOf.Error);
                    StopMergeByErrorWhileMerging(null);
                    return;
                }

                //Apply the new UV map merged using modification of all UV vertex of each submesh, apply all atlas too
                smrender.sharedMesh.uv = newUvMapForCombinedMesh.ToArray();
                ...
                //Generate the Stats text
                GenerateStatsTextAndFillIt(nameOfThisMerge, "Just Material Colors", verticesCount, timeOfStart, timeOfEnd, mergedMeshes, meshesBefore, drawCallReduction, materialCount, originalUvLenght, newUvMapForCombinedMesh.Count);
            ...
            try
            {
                ...
                foreach (SkinnedMeshRenderer meshRender in skinnedMeshesToMerge)
                {
                    ...
                    for (int i = 0; i < meshRender.sharedMesh.subMeshCount; i++)
                    {
                        ...
                        Vector2[] uvMapOfThisSubMesh = combineInstance.mesh.GetSubmesh(i).uv;
        ...
        private void DoUndoCombineMeshs(bool runMonoIl2CppGc, bool runUnityGc, bool errorOcurred)
        {
            ...
            //Show alert
            if (errorOcurred == false)
                ShowUnityAlertDialog("Done", "The last merge performed on this character was undone.", "Ok");
        ...
        private void ValidateAllVariables()
        {
            ...
            if (CurrentRP.haveAnotherSrpPackages == true && allInOneParams.useDefaultMainTextureProperty == true)
            {
                allInOneParams.mainTexturePropertyToFind = "_BaseMap";
                allInOneParams.mainTexturePropertyToInsert = "_BaseMap";
            }
            if (CurrentRP.haveAnotherSrpPackages == true && justMaterialColorsParams.useDefaultColorProperty == true)
            {
            ...
            if (CurrentRP.haveAnotherSrpPackages == false && allInOneParams.useDefaultMainTextureProperty == true)
            {
                allInOneParams.mainTexturePropertyToFind = "_MainTex";
                allInOneParams.mainTexturePropertyToInsert = "_MainTex";
            }
            if (CurrentRP.haveAnotherSrpPackages == false && justMaterialColorsParams.useDefaultColorProperty == true)
            {
        ...
        private void StopMergeByErrorWhileMerging(Exception exception)
        {
            ...
            if (exception != null)
            {
                ...
                //Show alert
                ShowUnityAlertDialog("Error", "An error occurred during this merge. Check the log or console for more details.", "Ok");
        ...
        private void ShowUnityAlertDialog(string title, string message, string okButtom)
        {
#if UNITY_EDITOR
            if (Application.isPlaying == true)
                return;
            EditorUtility.DisplayDialog(title, message, okButtom);
#endif
        }
        ...
        private void SaveMergeAsPrefab(string folderNameToSave, string prefabName, GameObject targetGo)
        {
            ...
            if (AssetDatabase.LoadAssetAtPath(fileDirectory, typeof(GameObject)) == null)
            {
                ...
                UnityEngine.Object prefab = PrefabUtility.CreatePrefab(fileDirectory, targetGo);
        ...
        private SkinnedMeshRenderer[] GetAllSkinnedMeshsValidatedToCombine(GameObject[] gameObjectsToCombine)
        {
            ...
            foreach (GameObject obj in gameObjectsToCombine)
            {
                ...
                if (meshRender != null)
                {
                    ...
                    if (meshRender.sharedMesh.blendShapeCount > 0 && ignoreBlendshapes == true)
                    {
                        LaunchLog("The mesh present in GameObject " + meshRender.gameObject.name + " was ignored during the merge process. Reason: " +
        ...
        private bool ExistsDifferentRootBones(SkinnedMeshRenderer[] skinnedMeshRenderers, bool launchLogs)
        {
            ...
            foreach (SkinnedMeshRenderer smr in skinnedMeshRenderers)
            {
                ...
                {
                    if (launchLogs == true)
                        LaunchLog("Different root bones were found in your character's meshes. This can cause blending problems and cause your character not to be as desired. If you still want to combine the meshes, disable the \"Only Equal Root Bones\" option, but be aware that the mesh resulting from the merge may not come out as expected, or contain artifacts or animation problems.", LogTypeOf.Error);
                    return true;
                }
            }
        ...
        private Texture2D GetValidatedCopyOfTexture(Material materialToFindTexture, string propertyToFindTexture, int widthOfCorrespondentMainTexture, int heightOfCorrespondentMainTexture, TexturesSubMeshes.UvBounds boundsUvValues, TextureType textureType, bool showProgress, float progress)
        {
            ...
            if (thisTexture.width != widthOfCorrespondentMainTexture || thisTexture.height != heightOfCorrespondentMainTexture)
                TextureResizer.Bilinear(thisTexture, widthOfCorrespondentMainTexture, heightOfCorrespondentMainTexture);
